...
`r nrow(mci_baseline_df)` participants were recruited in this study as indicated by the initial dataset (mci_baseline_df).  
`r nrow(mci_baseline_df) - nrow(mci_free_baseline_df)` participants were excluded from the study because they already had the MCI at the study baseline. Among those who are included, `r nrow(mci_new_beseline_df)` participants developed MCI.  
The average baseline age for the eligible study participants was `r round(mean(pull(mci_free_baseline_df, current_age)), 2)` and the proportion of women who are APOE4 carriers is `r women_apoe4`.
